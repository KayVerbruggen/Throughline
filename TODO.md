...
- [ ] In-app LLM features (suggest guards/effects, draft a prototype, reverse-engineer a codebase)
  ...
  - Next: NL authoring (F) — one box that drafts the right artifact kind (slots + trace) from a sentence, validate → preview → accept; then prototype drafting and reverse-engineering (B/C).
