...
- [ ] In-app LLM features (suggest guards/effects, draft a prototype, reverse-engineer a codebase)
  ...
  - Next: **"Formalize all use cases"** — a sequential batch over every flow, threading accepted variables forward so later flows reuse earlier ones. After that, the model critic (E) and NL authoring (F).
